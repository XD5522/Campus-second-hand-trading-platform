...
public class AdminLoginController {
     ...
     */
    @PostMapping("/login")
    public CommonResult<?> Login(HttpServletRequest request, @RequestBody AdminDto adminDto){
        ...
        if(administrators==null){
            log.info("管理员不存在"+adminDto.getAdminAccount());
            ...
            if(administrators.getAdminPassword().equals(md5psw) ){
                log.info("密码正确");
                token = jwtUtils.adminToken(administrators,adminDto.getAdminAccount(), 3600L);
